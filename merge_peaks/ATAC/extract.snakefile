...
rule extract_bed:
    ...
    shell:
        ...
        find . -type f -name "peaks.bed" | grep 'outs/peaks.bed' | while read -r file; do
...
rule extract_metadata:
    ...
    shell:
        ...
        find . -type f -name "singlecell.csv" | while read -r file; do
            directory=$(dirname "$file")
            filename=$(basename "$file")
...
rule extract_h5:
    ...
    shell:
        ...
        find . -type f -name "filtered_peak_bc_matrix.h5" | while read -r file; do
            directory=$(dirname "$file")
            filename=$(basename "$file")
...
rule extract_frags:
    ...
    shell:
        ...
        find . -type f -name "fragments.tsv.gz" | while read -r file; do
            directory=$(dirname "$file")
            filename=$(basename "$file")
        ...
        find . -type f -name "fragments.tsv.gz.tbi" | while read -r file; do
            directory=$(dirname "$file")
            filename=$(basename "$file")
